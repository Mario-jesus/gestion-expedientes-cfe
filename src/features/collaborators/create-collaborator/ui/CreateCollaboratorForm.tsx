...
import type { CreateCollaboratorDto, ContractType } from '@entities/collaborator';
import { useCreateCollaborator } from '../model/useCreateCollaborator';
import { catalogsApi } from '@entities/collaborator';
import type { Area, Adscripcion, Puesto } from '@entities/collaborator';
import styles from './CreateCollaboratorForm.module.scss';
...
export function CreateCollaboratorForm({
  ...
  const { createCollaborator, isLoading, errors, submitError, clearFieldError } =
    useCreateCollaborator(onSuccess);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    await createCollaborator(dataToSubmit);
  };
    ...
    <form className={styles.form} onSubmit={handleSubmit}>
      <div className={styles.section}>
        ...
        <div className={styles.formRow}>
          <div className={styles.formGroup}>
            <label htmlFor="rpe">
              RPE <span className={styles.required}>*</span>
            </label>
            <input
              id="rpe"
              name="rpe"
              ...
              value={formData.rpe}
              onChange={handleChange}
              placeholder="Ej: RPE001234"
              disabled={isLoading || loadingCatalogs}
              className={errors.rpe ? styles.inputError : ''}
              style={{ textTransform: 'uppercase' }}
            />
            {errors.rpe && <span className={styles.error}>{errors.rpe}</span>}
            <span className={styles.hint}>
              Registro de Personal de Empleados
            </span>
          </div>
          ...
          <div className={styles.formGroup}>
            ...
            <input
              ...
              style={{ textTransform: 'uppercase' }}
            />
            {errors.rtt && <span className={styles.error}>{errors.rtt}</span>}
            <span className={styles.hint}>
      ...
      <div className={styles.section}>
        <h3 className={styles.sectionTitle}>Datos Fiscales y de Seguridad Social</h3>
